 ...
 * A collection of useful social media provider utilities. Currently, this module
 * only supports
 * Twitter and the following features:
 *
 * - Logging into Twitter and authorizing the application through the OAuth
 * protocol.
 ...
 *
 * To use a social media provider, a user must log in and authorize the
 * application to perform
 * certain actions, such as accessing profile information or posting messages.
 *
 * Call `authorize` to prompt the user to login and authorize the application.
 * For Twitter, a
 * dialog box will appear asking the user for their permission and login
 * credentials.
 *
 * Before calling `authorize`, the application will need a consumer key and
 * secret provided by the
 * social media service provider for the OAuth protocol, used when calling
 * `create`.
 ...
 * ## Example
 *
 * This example authorizes the application, posts a message to the user's Twitter
 * account, then
 ...
 *      // Note: share() automatically calls authorize() so an explicit call as
 ...
 * @param {String} [settings.site="twitter"] Site to access. Only 'twitter' is
 * working.
 * @param {String} settings.consumerSecret Shared secret used to authenticate the
 * key.
 * @param {String} settings.consumerKey Key used to identify the client to the
 * service provider.
...
exports.create = function(settings) {
  ...
  return urls == null ? (alert("The Social.js module does not support " + site + " yet!"), null) : {
     ...
     * If successful, the client will receive an access token, which will be
     * saved for future
     * usage.
     * Call this function after opening the parent view or else the authorize UI
     * window will
     * appear in the background behind the view and unusable to the user.
     * @param {Function} [callback] Callback function executed after successfully
     * retrieving
     * an access token.
     */
    ...
    /**
     * @method share
     * Sends an update to the service provider. Implicitly calls authorize() to
     * authorize your app.
     * @param {...*} options Update parameters.
     * @param {String} options.message Message to send.
     * @param {Function} [options.success] Callback function executed after a
     * successful update.
     * @param {Function} [options.error] Callback function executed after a
     * failed update.
     */
